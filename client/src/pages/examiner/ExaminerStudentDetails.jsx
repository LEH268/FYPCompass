          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
              {studentConsultations.length === 0 ? (
                <p className="text-sm text-slate-500 text-center">No consultations logged.</p>
              ) : studentConsultations.map(c => (
                <div key={c.id} className="border-l-2 border-purple-200 pl-4 py-1">
                  <p className="text-xs font-bold text-slate-400">{c.date}</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          {student.finalFeedback && (
            <div className="bg-purple-50 rounded-2xl shadow-sm border border-purple-200 p-6">
              ...
              <p className="text-sm text-purple-900 leading-relaxed">"{student.finalFeedback}"</p>
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div className="p-6 space-y-6 max-h-[600px] overflow-y-auto">
              {studentSubmissions.length === 0 ? (
                <div className="text-center text-slate-500 py-6">No deliverables uploaded by this candidate yet.</div>
                  ...
                  <div className="p-5 bg-white flex justify-between items-start border-b border-slate-100">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">Milestone: {sub.milestone}</p>
                        {sub.studentMessage && (
                           <p className="text-xs text-slate-600 mt-2 bg-slate-50 p-2 rounded">Student Note: "{sub.studentMessage}"</p>
                        )}
                      </div>
                    </div>
                    <button
                  ...
                  {sub.feedback && (
                    <div className="p-5 bg-indigo-50/50 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-700 italic">"{sub.feedback}"</p>
                      </div>
                    </div>
                  )}
                </div>
