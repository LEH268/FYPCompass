  ...
  if (isSubmitted) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] animate-in zoom-in duration-300">
        ...
        <p className="text-slate-500 mt-2 font-medium">The system is finalizing {student.name}'s result. Redirecting to dashboard...</p>
      </div>
    );
  }

  if (!student) return <div className="p-8 text-center font-bold">Candidate not found.</div>;
        ...
        <form className="p-6 space-y-8">
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mb-2 font-medium">Clarity, ability to answer questions, and slide quality.</p>
              <textarea rows="2" placeholder="Examiner comments..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-purple-500 resize-none font-medium text-slate-700"></textarea>
            </div>
            <div className="w-full md:w-32 flex-shrink-0">
              <label className="block text-sm font-bold text-slate-800 mb-1 text-right">Score / 20</label>
              <input type="number" max="20" min="0" value={grades.presentation} onChange={(e) => setGrades({...grades, presentation: e.target.value})} className="w-full p-3 bg-white border border-slate-300 rounded-lg text-lg font-bold text-center outline-none focus:border-purple-600 focus:ring-1 focus:ring-purple-600 shadow-inner text-purple-700" />
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-slate-100">
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mb-2 font-medium">Code quality, system complexity, and successful deployment.</p>
              <textarea rows="2" placeholder="Examiner comments..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-purple-500 resize-none font-medium text-slate-700"></textarea>
            </div>
            <div className="w-full md:w-32 flex-shrink-0">
              <label className="block text-sm font-bold text-slate-800 mb-1 text-right">Score / 50</label>
              <input type="number" max="50" min="0" value={grades.technical} onChange={(e) => setGrades({...grades, technical: e.target.value})} className="w-full p-3 bg-white border border-slate-300 rounded-lg text-lg font-bold text-center outline-none focus:border-purple-600 focus:ring-1 focus:ring-purple-600 shadow-inner text-purple-700" />
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-slate-100">
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mb-2 font-medium">Thesis formatting, literature review, and accurate citations.</p>
              <textarea rows="2" placeholder="Examiner comments..." className="w-full p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-purple-500 resize-none font-medium text-slate-700"></textarea>
            </div>
            <div className="w-full md:w-32 flex-shrink-0">
              <label className="block text-sm font-bold text-slate-800 mb-1 text-right">Score / 30</label>
              <input type="number" max="30" min="0" value={grades.documentation} onChange={(e) => setGrades({...grades, documentation: e.target.value})} className="w-full p-3 bg-white border border-slate-300 rounded-lg text-lg font-bold text-center outline-none focus:border-purple-600 focus:ring-1 focus:ring-purple-600 shadow-inner text-purple-700" />
            </div>
          </div>
          
          <div className="pt-6 border-t border-slate-100 flex flex-col sm:flex-row justify-end gap-4">
            <button type="button" onClick={handleSaveDraft} className="flex items-center justify-center px-6 py-3 bg-white border border-slate-300 text-slate-700 font-bold rounded-lg hover:bg-slate-50 transition-all shadow-sm">
              <Clock className="w-5 h-5 mr-2" /> Save Draft
            </button>
            <button type="button" onClick={handleFinalize} disabled={totalScore === 0} className="flex items-center justify-center px-8 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 transition-all shadow-md active:scale-95 disabled:bg-slate-300 disabled:cursor-not-allowed">
