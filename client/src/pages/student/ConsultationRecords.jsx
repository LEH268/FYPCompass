      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input type="text" placeholder="Search topics..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-indigo-600 outline-none transition-all" />
          </div>
        </div>
              ...
              <div key={log.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active cursor-pointer" onClick={() => toggleExpand(log.id)}>
                ...
                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded flex-shrink-0 ml-2 ${log.status === 'Logged' ? 'bg-emerald-100 text-emerald-700' : log.status === 'Upcoming' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'}`}>
                      {log.status}
                    </span>
                  </div>
                  ...
                  {expandedId === log.id ? (
                    <div className="animate-in slide-in-from-top-2 duration-300">
                      <div className="mt-3">
                        <p className="text-xs font-bold text-slate-700 mb-1">Notes / Summary:</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <form onSubmit={handleLogSubmit} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex justify-end space-x-3">
                ...
                <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700">Save Record</button>
              </div>
            </form>
          </div>
        </div>
