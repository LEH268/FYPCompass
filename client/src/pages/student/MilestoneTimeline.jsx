...
export default function MilestoneTimeline() {
  ...
  // Dynamically calculate status based on actual student progress
  const getDynamicStatus = (requiredProgress) => {
    if (myData.progress > requiredProgress) return "completed";
    if (myData.progress === requiredProgress || (myData.progress < requiredProgress && myData.progress >= requiredProgress - 25)) return "in-progress";
    return "pending";
  };
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      ...
      case "in-progress": return <Clock className="text-indigo-500 bg-white rounded-full shadow-sm" size={28} />;
            ...
            <div key={milestone.id} className="relative pl-8">
              ...
              <div className={`p-5 rounded-xl border transition-all ${
                ...
                <div className="flex justify-between items-start mb-2">
                  <h3 className={`text-lg font-bold ${milestone.status === 'pending' ? 'text-slate-600' : 'text-slate-800'}`}>{milestone.title}</h3>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-sm font-medium">
                  ...
                  {milestone.status === "completed" && (
                    <button 
                      onClick={() => navigate('/student/proposal')}
                      className="text-emerald-700 bg-emerald-100 hover:bg-emerald-200 px-4 py-2 rounded-lg flex items-center justify-center font-bold text-sm transition-colors w-full sm:w-auto"
                    >
