    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium">Semester: <span className="font-bold text-slate-700 ml-1">Aug 2026</span></p>
        </div>
        <button onClick={() => setShowExportModal(true)} className="flex items-center justify-center px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors shadow-sm">
              ...
              {students.map((s) => (
                <tr key={s.id} onClick={() => navigate(`/coordinator/students/${s.id}`)} className="hover:bg-slate-50/50 transition-colors cursor-pointer group">
                  <td className="p-4 pl-5">
                    ...
                    <div className="text-xs text-slate-500 font-medium mt-0.5">Sup: {s.supervisorName}</div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4 w-1/4">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1 bg-slate-200 rounded-full h-2 overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-1000 ${s.progress === 100 ? 'bg-purple-500' : s.status === 'At Risk' ? 'bg-rose-500' : 'bg-indigo-500'}`} style={{ width: `${s.progress}%` }}></div>
                      </div>
                    </div>
                  </td>
              ...
              <>
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">Export student progress data</p>
                  </div>
