  ...
  if (!supervisor) {
    return (
      <div className="p-8 text-center animate-in fade-in">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 px-4 py-2 bg-indigo-600 text-white font-bold rounded-lg shadow-sm active:scale-95">Go Back</button>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[500px] overflow-y-auto">
          {supervisees.length === 0 ? (
            <div className="p-8 text-center text-slate-500 border border-dashed border-slate-300 rounded-xl font-bold bg-slate-50/50">No students currently assigned to this supervisor.</div>
          ) : supervisees.map(s => (
             <div key={s.id} onClick={() => navigate(`/coordinator/students/${s.id}`)} className="p-5 border border-slate-200 rounded-xl hover:border-indigo-400 transition-all cursor-pointer flex flex-col sm:flex-row justify-between sm:items-center bg-white shadow-sm hover:shadow-md gap-4">
                <div>
