      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Review {student.name}'s deliverables, messages, and consultation history.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div className="flex flex-col items-center text-center pb-6 border-b border-slate-100">
              ...
              <span className={`px-3 py-1 mt-3 text-xs font-bold rounded-full uppercase tracking-wider ${student.status === 'On Track' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                {student.status}
              </span>
            </div>
            <div className="pt-6 space-y-3">
              ...
              <div className="mt-6 pt-4 border-t border-slate-100">
                ...
                <div className="w-full bg-slate-100 rounded-full h-2.5 overflow-hidden mb-2">
                  <div className={`h-2.5 rounded-full transition-all duration-1000 ${student.status === 'At Risk' ? 'bg-rose-500' : 'bg-indigo-600'}`} style={{ width: `${student.progress}%` }}></div>
                </div>
                <p className="text-xs text-slate-500 text-right">Current Phase: <span className="font-semibold text-slate-700">{student.stage}</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
              {studentConsultations.length === 0 ? (
                <p className="text-sm text-slate-500 text-center">No consultations logged yet.</p>
                ...
                <div key={c.id} className="border-l-2 border-indigo-200 pl-4 py-1">
                  ...
                  <p className="text-sm font-semibold text-slate-700">{c.topic}</p>
                  ...
                  {sub.feedback && (
                    <div className="p-5 bg-indigo-50/30 flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-indigo-900 leading-relaxed">"{sub.feedback}"</p>
                      </div>
                    </div>
                  )}
