    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Schedule meetings or log summaries for your supervisees.</p>
        </div>
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 shadow-md transition font-bold active:scale-95">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          {sortedLogs.map((log) => {
            ...
            return (
              <div key={log.id} className="border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-indigo-300 transition-all cursor-pointer group bg-white" onClick={() => toggleExpand(log.id)}>
                ...
                {expandedId === log.id && (
                  <div className="animate-in slide-in-from-top-2 duration-300 border-t border-slate-100 pt-4 mt-2">
                    ...
                    <div className="flex justify-center mt-3">
                      <button className="text-xs font-bold text-slate-400 hover:text-indigo-600 flex items-center transition-colors">
          ...
          {sortedLogs.length === 0 && (
            <div className="text-center text-slate-500 font-bold py-10 w-full bg-slate-50 rounded-xl border border-dashed border-slate-300">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50">
              ...
              <p className="text-sm text-slate-500 font-medium">Document the consultation details and assign action items.</p>
            </div>
            <form onSubmit={handleLogSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input name="topic" type="text" required placeholder="e.g. Chapter 3 Methodology" className="w-full px-4 py-3 border border-slate-300 shadow-sm rounded-xl outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 text-sm font-medium" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Meeting Summary</label>
                <textarea name="summary" rows="3" required placeholder="What was evaluated or discussed?" className="w-full px-4 py-3 border border-slate-300 shadow-sm rounded-xl outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 resize-none text-sm font-medium"></textarea>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Action Items</label>
                <textarea name="actionItems" rows="2" required placeholder="Tasks assigned to the student." className="w-full px-4 py-3 border border-slate-300 shadow-sm rounded-xl outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 resize-none text-sm font-medium"></textarea>
              </div>
