...
export default function StudentProgress() {
  ...
  // Fetch all active students belonging to Dr. Alan Turing (F01)
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">Track the progress and health of your {myStudents.length} assigned students.</p>
        </div>
        <div className="flex space-x-3">
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Search student..." className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm font-medium focus:ring-2 focus:ring-indigo-600 outline-none w-full sm:w-64" />
          </div>
          <button className="flex items-center px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 text-sm font-bold shadow-sm transition-colors">
        ...
        {["All", "On Track", "At Risk"].map((tab) => (
          <button
            ...
            className={`px-4 py-2 text-sm font-bold border-b-2 transition-colors ${
              filter === tab ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"
            }`}
          >
            {tab}
          </button>
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredStudents.length === 0 ? (
           <div className="p-8 text-center text-slate-500 font-bold border border-dashed border-slate-300 rounded-xl bg-slate-50">
          ...
          >
            <div className="flex-1">
              <div className="flex items-center mb-2">
                ...
                <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${student.status === 'On Track' ? 'bg-emerald-100 text-emerald-700' : student.status === 'Graded' ? 'bg-purple-100 text-purple-700' : 'bg-rose-100 text-rose-700'}`}>
                  {student.status}
                </span>
              </div>
              <p className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-wide">ID: {student.id}</p>
            ...
            <div className="md:w-64 flex-shrink-0 border-l md:border-l border-slate-100 md:pl-6 space-y-4">
              ...
              <div className="flex space-x-2 pt-2">
                <div className="w-full py-2 bg-white border border-slate-200 text-indigo-700 text-xs font-bold rounded-lg group-hover:bg-indigo-600 group-hover:text-white transition-colors flex justify-center items-center shadow-sm">
